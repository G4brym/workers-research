...
function createCustomRenderer(): Renderer {
  ...
  renderer.list = function(token: ListToken): string {
    ...
    const classes = token.ordered
      ? 'list-decimal list-inside space-y-2 mb-6 text-gray-700 ml-4'
      : 'list-disc list-inside space-y-2 mb-6 text-gray-700 ml-4';
  ...
  renderer.link = function(token: Token & { href: string; title?: string }): string {
    const text = this.parser.parseInline(token.tokens || []);
  ...
  renderer.image = function(token: Token & { href: string; title?: string; text?: string }): string {
    const titleAttr = token.title ? ` title="${token.title}"` : '';
...
// Main function to render markdown with custom styling
export function renderMarkdownReport(markdownContent: string): string {
  ...
  // Render the markdown
  const htmlContent: string = marked.parse(markdownContent);

  // Wrap the content in the report container to match your structure
  return `
...
// Advanced function with configuration options
export function renderMarkdownReportWithConfig(
  markdownContent: string,
  config: ReportRendererConfig = {}
): string {
  const {
    containerClasses = 'prose prose-lg max-w-none',
    contentClasses = 'bg-white rounded-lg shadow-sm border border-gray-200 p-8',
...
// Simple test function to debug issues
export function testRenderer(markdownContent: string): void {
  console.log('Input markdown:', markdownContent);
  const result = renderMarkdownReportContent(markdownContent);
...
// Example 1: Full wrapped report
const htmlReport: string = renderMarkdownReport(yourMarkdownString);
document.getElementById('report-container')!.innerHTML = htmlReport;

// Example 2: Just the content
const htmlContent: string = renderMarkdownReportContent(yourMarkdownString);
document.querySelector('.report-content')!.innerHTML = htmlContent;

// Example 3: Debug what's happening
testRenderer('# Test Heading\n\nThis is a paragraph with **bold text**.');
*/
